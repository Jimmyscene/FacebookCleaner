$(function() {
    ...
    function checkNewInstall() {
        chrome.storage.sync.get("helpMessageShown", function(results) {
            if (results["helpMessageShown"] == undefined) {
                showHelpMessage();
            }
        });
    }
    ...
    function checkLoggedIn(newTab,successCallback,notLoggedInCallback,failureCallback){
        chrome.tabs.onUpdated.addListener( function(tabId, changeInfo, tab) {
            if(tabId == newTab.id) {
                if(changedInfo["title"] == "Facebook"){
                    successCallback();
                }
                else if(changedInfo["title"] == "Facebook - Log In or Sign Up") {
                    notLoggedInCallback();
                }
